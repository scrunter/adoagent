...
When recovery requires a corrected toolkit release, run that release with the original ConfigId, original setup inputs, and `-Resume`. The verified toolkit package path may be rebound and setup persists the new path after confirmation. Before any escrow envelope, manifest, or rollback artifact exists, an invalid protector-group name may also be corrected to a validated domain security group in the current logon token. After escrow exists, an alternate name is accepted only when it resolves to the exact SID recorded in the manifest; a different protector SID is rejected. Do not copy new binaries over an older release directory merely to preserve its pathname.
