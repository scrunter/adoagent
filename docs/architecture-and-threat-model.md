...
4. The current node seals directly under the elevated provisioning operator. On each passive node, ordinary WinRM stages the envelope and starts the fixed helper under the elevated installer identity. The helper receives an in-memory protector-group credential through an anonymous stdin pipe, creates an interactive logon token with `LogonUser`, and impersonates that token only while unwrapping DPAPI-NG. After reverting, it writes only a temporary classic-DPAPI ciphertext; the elevated installer validates its fingerprint and atomically installs it at the fixed ProgramData path. Each resulting ciphertext is usable only on that node. No CredSSP configuration, alternate-credential process launch, or reusable credential file is created.
...
| Node enrollment | elevated installer through WinRM; helper thread briefly impersonates a fresh protector-group logon on passive nodes | DPAPI-NG unwrap while impersonated; node-local ciphertext creation/install and configuration after reverting to the elevated installer |
