...
A changed URL, pool, name, root, node set, service identity, Microsoft agent-package choice, insecure-URL policy, or ConfigId is rejected. A corrected toolkit release may be used from its new extracted path. Before any envelope, manifest, or rollback key artifact exists, a bad protector-group name may also be corrected to a validated domain security group in the current logon token. Both permitted corrections are persisted only after confirmation; the protector cannot change after key artifacts exist. Use `-ReplaceExistingAgent` only when approved change control has confirmed that an existing Offline server-side registration with the same name is intentionally being replaced.
