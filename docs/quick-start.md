...
A changed URL, pool, name, root, node set, service identity, Microsoft agent-package choice, insecure-URL policy, or ConfigId is rejected. A corrected toolkit release may be used from its new extracted path. Before escrow exists, a bad protector-group name may be corrected to a validated domain security group in the current logon token. After escrow exists, only an alternate name that resolves to the manifest's existing protector SID is accepted; the SID itself cannot change. Permitted corrections are persisted only after confirmation. Use `-ReplaceExistingAgent` only when approved change control has confirmed that an existing Offline server-side registration with the same name is intentionally being replaced.
